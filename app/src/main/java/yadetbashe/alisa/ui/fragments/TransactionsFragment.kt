...
class TransactionsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = TransactionAdapter(
            onItemClicked = { transaction ->
                // منوی گزینه‌ها: ویرایش / علامت‌گذاری / حذف
                showOptionsDialog(transaction)
            },
            onItemLongClicked = { transaction ->
                // میان‌بر: لمس طولانی = حذف
    ...
    /**
     * منوی گزینه‌های تراکنش: ویرایش، علامت‌گذاری پرداخت‌شده/نشده، حذف
     */
    private fun showOptionsDialog(transaction: Transaction) {
        val options = arrayOf(
            getString(R.string.edit),
            if (transaction.isPaid) getString(R.string.mark_unpaid) else getString(R.string.mark_paid),
            getString(R.string.delete)
        )
        MaterialAlertDialogBuilder(requireContext())
            .setTitle(
                transaction.description.ifEmpty { getString(R.string.options) }
            )
            .setItems(options) { _, which ->
                when (which) {
                    0 -> TransactionDialogFragment.newInstance(transaction.id)
                        .show(parentFragmentManager, "TransactionDialog")

                    1 -> viewModel.togglePaid(transaction)

                    2 -> confirmDelete(transaction)
                }
            }
